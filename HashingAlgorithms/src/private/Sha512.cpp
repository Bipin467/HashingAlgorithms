...
uint16_t HashingAlgorithm::SHA512::CalculateK(uint64_t bitLength)
{
	return (1024 - ((bitLength + 1 + 128) % 1024)) % 1024;
}

void HashingAlgorithm::SHA512::Transform()
{
	uint64_t w[80];
	for (int i = 0; i < 16; i++) {
	...
	h = m_tempHash[7];
	for (int i = 0; i < 80; i++) {
		T1 = h + this->BSIG1(e) + this->CH(e, f, g) + SHA512CONSTS::K[i] + w[i];
...
uint64_t HashingAlgorithm::SHA512::CH(uint64_t x, uint64_t y, uint64_t z)
{
	return (x&y) ^ ((~x)&z);
}

uint64_t HashingAlgorithm::SHA512::Maj(uint64_t x, uint64_t y, uint64_t z)
{
	return (x&y) ^ (x&z) ^ (y&z);
}

uint64_t HashingAlgorithm::SHA512::BSIG0(uint64_t x)
{
	return ROTR64(x, 28) ^ ROTR64(x, 34) ^ ROTR64(x, 39);
}

uint64_t HashingAlgorithm::SHA512::BSIG1(uint64_t x)
{
	return ROTR64(x, 14) ^ ROTR64(x, 18) ^ ROTR64(x, 41);
}

uint64_t HashingAlgorithm::SHA512::SSIG0(uint64_t x)
{
	return ROTR64(x, 1) ^ ROTR64(x, 8) ^ (x >> 7);
}

uint64_t HashingAlgorithm::SHA512::SSIG1(uint64_t x)
{
	return ROTR64(x, 19) ^ ROTR64(x, 61) ^ (x >> 6);
}

HashingAlgorithm::SHA512 & HashingAlgorithm::SHA512::GetInstance()
{
	static SHA512 instance;
	return instance;
}

void HashingAlgorithm::SHA512::Convert64BitHexToStr(uint64_t val, char * data)
{
...
char * HashingAlgorithm::SHA512::ComputeHash(const std::string & data)
{
	...
	memcpy(this->m_tempHash, SHA512CONSTS::H, sizeof(m_tempHash));
	...
	this->m_finalStringMessage[128] = 0x00;
